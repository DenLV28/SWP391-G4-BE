...
export default function Register({ onRegister, setView, users }: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!plateNumber.trim()) tempErrors.plateNumber = 'Vui lòng nhập biển số xe.';
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
            ...
            <FormInput
              label="Biển số xe"
              value={plateNumber}
              onChange={(e: any) => { setPlateNumber(e.target.value); if (errors.plateNumber) setErrors(p => ({ ...p, plateNumber: undefined })); }}
