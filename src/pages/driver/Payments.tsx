...
import { Copy, Receipt, X, Trash2, FileText, Printer } from 'lucide-react';
...
export default function Payments({
  ...
  const [cryptoPayment, setCryptoPayment] = useState<{ payment: Payment; method: PaymentMethod } | null>(null);
  ...
  const handlePayConfirm = async (id: string) => {
    ...
    if (matchedMethod === 'VNPay') {
      ...
      try {
        setLoadingVNPay(id);
        const url = await createVNPayPayment(id, payment.totalAmount, `Thanh toan phi giu xe ${id}`);
    ...
    if (matchedMethod === 'Crypto') {
      const payment = filtered.find((p) => p.id === id);
      if (payment) setCryptoPayment({ payment, method: matchedMethod });
      return;
    }
    if (matchedMethod === 'QR Banking') {
      const payment = filtered.find((p) => p.id === id);
      if (payment) setQrPayment(payment);
      return;
    }
  ...
  return (
    <div className="space-y-6">
              ...
              {payment.status === 'Unpaid' ? (
                payment.totalAmount <= 0 ? (
                  ...
                  <div className="space-y-3 pt-1">
                    ...
                    {selectedMethods[payment.id] === 'VNPay' && (
      ...
      {cryptoPayment && (
        <CryptoPayModal
          amountVND={cryptoPayment.payment.totalAmount}
          onConfirm={() => {
            onConfirmPayment(cryptoPayment.payment.id, cryptoPayment.method);
      ...
      {qrPayment && (
        <VietQRModal
          amount={qrPayment.totalAmount}
          description={`ParkFlow ${qrPayment.id} ${qrPayment.ticketCode}`}
          onConfirm={() => {
            onConfirmPayment(qrPayment.id, 'QR Banking');
            setQrPayment(null);
          }}
...
const CRYPTO_NETS_P = [
  {
    ...
    label: 'USDT',
    chain: 'TRC20',
    chainCls: 'bg-red-100 text-red-700',
    address: 'TParkFlowDemoPaymentCryptoAddr0001',
    rate: 25000,
    symbol: 'USDT',
    dec: 2,
  },
  {
    ...
    label: 'USDT',
    chain: 'BEP20',
    chainCls: 'bg-yellow-100 text-yellow-700',
    address: '0xDe77F8Cab30e5E1B8B0d3d8E1E0f6a3b5c2e8190',
    rate: 25000,
    symbol: 'USDT',
    dec: 2,
  },
  {
    ...
    label: 'ETH',
    chain: 'Ethereum',
    chainCls: 'bg-indigo-100 text-indigo-700',
    address: '0xDe77F8Cab30e5E1B8B0d3d8E1E0f6a3b5c2e8190',
    rate: 3000000,
    symbol: 'ETH',
    dec: 6,
...
function CryptoPayModal({
  ...
  const [netId, setNetId] = useState<typeof CRYPTO_NETS_P[number]['id']>('usdt-trc20');
  const [copied, setCopied] = useState(false);

  const net = CRYPTO_NETS_P.find((n) => n.id === netId)!;
  const cryptoAmount = (amountVND / net.rate).toFixed(net.dec);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-sm rounded-3xl border border-violet-100 bg-white p-6 shadow-2xl space-y-4">
        <button
          type="button"
          onClick={onClose}
          className="absolute right-4 top-4 cursor-pointer text-slate-400 hover:text-slate-600"
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-violet-100 text-violet-700 text-xl font-black">₿</div>
          <div>
            <h3 className="text-base font-bold text-slate-800">Thanh toán Crypto</h3>
            <p className="text-[10px] text-slate-400">Quét mã QR bằng ví điện tử của bạn</p>
          </div>
        </div>
        ...
        <div className="flex gap-1.5 flex-wrap">
          {CRYPTO_NETS_P.map((n) => (
            ...
            >
              {n.label}
              <span className={`px-1 py-0.5 rounded text-[8px] font-black ${n.chainCls}`}>{n.chain}</span>
            </button>
          ))}
        </div>

        <div className="flex justify-center">
          <div className="rounded-2xl bg-white p-3 border-2 border-violet-200 shadow-inner">
            <QRCode value={net.address} size={160} level="M" fgColor="#5b21b6" />
          </div>
        </div>

        <div className="rounded-xl bg-violet-50 border border-violet-100 p-3 space-y-2">
          <div>
            <p className="text-[9px] font-bold uppercase tracking-wide text-slate-400">Địa chỉ ví ({net.chain})</p>
            <p className="text-[9px] font-mono text-slate-700 break-all mt-0.5">{net.address}</p>
            <button
              type="button"
              onClick={handleCopy}
              className={`mt-1.5 flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded transition ${
                copied ? 'text-emerald-600 bg-emerald-50' : 'text-violet-600 hover:bg-violet-100'
              }`}
            >
          ...
          <div className="border-t border-violet-100 pt-2">
            <p className="text-[9px] font-bold uppercase tracking-wide text-slate-400">Số tiền cần chuyển</p>
            <p className="text-2xl font-black text-violet-700 mt-0.5">
              {cryptoAmount} <span className="text-sm">{net.symbol}</span>
            </p>
            <p className="text-[8px] text-slate-400">≈ {amountVND.toLocaleString('vi-VN')}đ · 1 {net.symbol} = {net.rate.toLocaleString('vi-VN')}đ</p>
          </div>
        </div>

        <div className="flex items-start gap-2 rounded-lg border border-amber-100 bg-amber-50 p-2 text-[9px] text-amber-700 font-medium">
          <span className="text-sm leading-none mt-0.5">⚠</span>
          Địa chỉ DEMO. Nhấn xác nhận để mô phỏng thanh toán thành công.
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className="w-1/2 rounded-xl bg-violet-600 py-2.5 text-xs font-bold text-white hover:bg-violet-500 transition"
          >
